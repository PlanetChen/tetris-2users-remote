...
author: cxx
modified: 2017.08.22
reason: modify program from 1 user to 2 users
...
UDP_BROADCAST_EVENT = USEREVENT + 2
WAIT_100_MS_BLOCK_EVENT = USEREVENT + 3
...
#cxx
logger =0 #cxx added for logging
# for 2 users
score_2 = 0  # 玩家得分记录
clearLineScore_2 = 0  # 玩家清除的方块行数
level_2 = 1  # 关卡等级
#cxx
...
#cxx  for 2 users
nowBlock_2 = None  # 当前下落中的方块
nextBlock_2 = None  # 下一个将出现的方块
fallSpeed_2 = 10  # 当前方块下落速度
beginFallSpeed_2 = fallSpeed_2  # 游戏初始时方块下落速度
speedBuff_2 = 0  # 下落速度缓冲变量
keyBuff_2 = None  # 上一次按键记录
#maxBlockWidth_2 = 10  # 舞台堆叠区X轴最大可容纳基础方块数
#maxBlockHeight_2 = 18  # 舞台堆叠区Y轴最大可容纳基础方块数
#blockWidth_2 = 30  # 以像素为单位的基础方块宽度
#blockHeight_2 = 30  # 以像素为单位的基础方块高度
blocks_2 = []  # 方块形状矩阵四维列表。第一维为不同的方块形状，第二维为每个方块形状不同的方向（以0下标起始，一共四个方向），第三维为Y轴方块形状占用情况，第四维为X轴方块形状占用情况。矩阵中0表示没有方块，1表示有方块。
stage_2 = []  # 舞台堆叠区矩阵二维列表，第一维为Y轴方块占用情况，第二维为X轴方块占用情况。矩阵中0表示没有方块，1表示有固实方块，2表示有活动方块。
gameOver_2 = False  # 游戏结束标志
...
randint_2_next_2_1_tmp = 0
randint_2_next_2_2_tmp = 0
...
#cxx
def printTxt_2(content, x, y, font, screen, color=(255, 255, 255)):
    '''显示文本
    args:
        content:待显示文本内容
        x,y:显示坐标
...
class blockSprite_2(object):
    ...
    def chgDirection(self, direction):
        ...
        args:
            direction:1为向右转，0为向左转。
        '''
        dirNumb = len(blocks_2[self.shape]) - 1
        if direction == 1:
            self.direction += 1
            if self.direction > dirNumb:
                self.direction = 0
    ...
    def clone(self):
        ...
        return:
            返回自身的克隆
        '''
        return blockSprite_2(self.shape, self.direction, point(self.xy.x, self.xy.y))

    def _getBlock(self):
        return blocks_2[self.shape][self.direction]
...
def sysInit():
    ...
    global  blocks_2, stage_2, gameOver_2, fallSpeed_2, beginFallSpeed_2, nowBlock_2, nextBlock_2, score_2, level_2, clearLineScore_2  #, pause_2
...
#    pause_2 = False
    ...
    clearLineScore_2 = 0
    beginFallSpeed_2 = 20
    fallSpeed_2 = beginFallSpeed_2 - level_2 * 2
    ...
    for y in range(len(sprite.block)):
        for x in range(len(sprite.block[y])):
            if sprite.block[y][x] == 1:
                ...
                if xInStage > maxBlockWidth - 1 or xInStage < 0:
                    return True
                if stage_2[yInStage][xInStage] == STATIC_BLOCK:
                    return True
    return False
...
#cxx
def updateStage_2(sprite, updateType=1):
    '''
    ...
    for y in range(len(sprite.block)):
        for x in range(len(sprite.block[y])):
            if sprite.block[y][x] == 1:
                if updateType == 0:
                    if stage_2[topY + y][topX + x] == FALLING_BLOCK:
                        stage_2[topY + y][topX + x] = EMPTY_CELL
                elif updateType == 1:
                    if stage_2[topY + y][topX + x] == EMPTY_CELL:
                        stage_2[topY + y][topX + x] = FALLING_BLOCK
                else:
                    stage_2[topY + y][topX + x] = STATIC_BLOCK
...
def updateScore_2(clearCount):
    ...
    return:
        当前游戏的最新积分
    '''
    global score_2, fallSpeed_2, level_2, clearLineScore_2
    ...
    score_2 += (clearCount + prizePoint) * level_2
    # 玩得再牛又有何用？ :)
    if score_2 > 99999999:
        score_2 = 0
    clearLineScore_2 += clearCount
    if clearLineScore_2 > 100:
        clearLineScore_2 = 0
        level_2 += 1
        if level_2 > (beginFallSpeed_2 / 2):
            level_2 = 1
            fallSpeed_2 = beginFallSpeed_2
        fallSpeed_2 = beginFallSpeed_2 - level_2 * 2
    return score_2
...
def drawStage(drawScreen):
    ...
    baseRect = 0 + X_2_ADD, 0, blockWidth * maxBlockWidth + 1, blockHeight * maxBlockHeight + 1  # 堆叠区方框
    ...
    for y in range(len(stage)):
        for x in range(len(stage[y])):
            baseRect = x * blockWidth + X_2_ADD , y * blockHeight, blockWidth, blockHeight
            if stage[y][x] == 2:
    ...
    if nextBlock != None:
        for y in range(len(nextBlock.block)):
            for x in range(len(nextBlock.block[y])):
                baseRect = 320 + x * blockWidth + X_2_ADD, 380 + y * blockHeight, blockWidth, blockHeight
...
#cxx  left frame, 2nd user's frame
    ...
    # 绘制堆叠区外框
    #cxx  drawScreen.fill((180, 200, 170))
    pg.draw.rect(drawScreen, staticColor, baseRect, 1)

    # 绘制堆叠区内的所有方块，包括下落方块形状
    for y in range(len(stage_2)):
        for x in range(len(stage_2[y])):
    ...
    # 绘制下一个登场的下落方块形状
    printTxt_2("Next:", 320, 350, defaultFont, backSurface, fontColor)
    if nextBlock_2 != None:
        for y in range(len(nextBlock_2.block)):
            for x in range(len(nextBlock_2.block[y])):
                baseRect = 320 + x * blockWidth , 380 + y * blockHeight, blockWidth, blockHeight
                if nextBlock_2.block[y][x] == 1:
                    pg.draw.rect(drawScreen, activeColor, baseRect)

    # 绘制关卡、积分、当前关卡消除整行数
    printTxt_2("Level:%d" % level_2, 320, 40, defaultFont, backSurface, fontColor)
    printTxt_2("Score:%d" % score_2, 320, 70, defaultFont, backSurface, fontColor)
    printTxt_2("Clear:%d" % clearLineScore_2, 320, 100, defaultFont, backSurface, fontColor)
    ...
    #cxx if pause_2:
    #cxx    printTxt_2("Game pausing", 230, 200, defaultFont, backSurface, fontColor)
    #cxx    printTxt_2("<PRESS RETURN TO CONTINUE>", 200, 260, defaultFont, backSurface, fontColor)
...
def process():
    ...
    if restart_key_pressed == 1 or wait_for_2_nowBlock_0 ==1 :
        ...
        if iamready == 1:
            #print('nowBlock_2 == None and iamready == 1')
            #print(str(randint_2_next_2_1) + ' / ' + str(randint_2_next_2_2))
            nextBlock_2 = blockSprite_2(randint_2_next_2_1, randint_2_next_2_2,
                                        point(maxBlockWidth + 4, maxBlockHeight))
            print("middle 4 ?")
        else:
            #print('nowBlock_2 == None and iamready == 0')
            nextBlock_2 = blockSprite_2(random.randint(0, len(blocks_2) - 1), random.randint(0, 3),
                                point(maxBlockWidth + 4, maxBlockHeight))
        # 每次生成新的下落方块形状时检测碰撞，如果新的方块形状一出现就发生碰撞，则显然玩家已经没有机会了。
         ...
         #cxx added
            ...
            if event.key == pg.K_ESCAPE:
                ...
                if gameOver:
                    ...
                    pause = True
                    return
            elif not gameOver and not pause and not gameOver_2:
            ...
            #cxx
                    ...
                    tmpBlock_2.chgDirection(1)
                elif event.key == pg.K_w:
                    tmpBlock_2.chgDirection(0)
            #cxx

    if not gameOver and not pause  and not gameOver_2:
        '''
        ...
        if speedBuff >= fallSpeed:
            ...
            if not checkDeany(tmpBlock):
                ...
                updateStage(nowBlock, 2)
                checkLine()
                nowBlock = None  #如果此时，下落已经相碰撞，但还没收到对方传来的随机数来，还不能生成新的block？cxx
...
class Server(threading.Thread):
    def __init__(self):  # 初始化和销毁对象函数.
        ...
        self.thStop = False
        print(str(self.ADDR))
        #print(socket.gethostbyname(self.sock.gethostname()))  # 得到本地ip
    ...
    def transMsg(self):
        ...
        if localIP == curAddr[0]: #if send from local IP, pass
            ...
            restart_received = 1
            #my_event = pg.event.Event(KEYARROWEVENT, {"key": K_b, "mod": 0, "unicode": u' '})
            #pg.event.post(my_event)
        elif data.find('r1') != -1:
            iamready = 1
            print('received: ' + data)
            ...
            randint_2_next_2_2_tmp = int(randint_2_next_2_2_str)
            my_event = pg.event.Event(KEYARROWEVENT, {"key": K_r, "mod": 0, "unicode": u' '})
            pg.event.post(my_event)
            print('middle 2?')
        elif data.find('hello') != -1:
            if  targetIP == curAddr[0]:# just receive the first time message
                pass
            else:
                targetIP = curAddr[0]
                my_event = pg.event.Event(KEYARROWEVENT, {"key": K_p, "mod": 0, "unicode": u' '})
                pg.event.post(my_event)

        #print '< ' + data + ' from address: ' + str(curAddr)
...
def udp_client_loop(ipaddress):
    global cli
    name = 'c2'
    cli = Client(ipaddress, name)
    cli.start()
...
def sendUDPbroadcastMsg():
    global localIP
    addr = ('<broadcast>', 21567)
...
def main():
    ...
    udp_server_loop()
    sendUDPbroadcastMsg()
    #udp_client_loop("192.168.1.1")
    pg.time.set_timer(UDP_BROADCAST_EVENT, 1000*60)
